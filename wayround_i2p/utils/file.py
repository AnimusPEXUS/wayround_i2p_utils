...
def _copytree_sub(
    ...
    full_src_dir = wayround_i2p.utils.path.abspath(src_dir)
    full_dst_dir = wayround_i2p.utils.path.abspath(dst_dir)
    ...
    if not os.path.isdir(full_src_dir):
        ...
        if create_if_not_exists_dir(full_dst_dir) != 0:
            ...
            for path, dirs, files in os.walk(
                ...
                path_dst = wayround_i2p.utils.path.join(
                    full_dst_dir,
                ...
                for i in dirs:
                    ...
                    joined = wayround_i2p.utils.path.join(path, i)
                    joined_dst = wayround_i2p.utils.path.join(path_dst, i)
...
def copytree(
    ...
    src_dir = wayround_i2p.utils.path.abspath(src_dir)
    dst_dir = wayround_i2p.utils.path.abspath(dst_dir)
...
def copy_file_or_directory(
    ...
    if os.path.islink(src_path):
        ret = copy_symlink(
            src_path,
            dst_dir,
            # wayround_i2p.utils.path.join(dst_dir, os.path.basename(src_path)),
...
def remove_if_exists(file_or_dir, remove_dead_symlinks=False, verbose=True):
    ...
    file_or_dir = wayround_i2p.utils.path.abspath(file_or_dir)
...
def cleanup_dir(dirname, verbose=True):
    ...
    for i in range(len(files)):
        files[i] = wayround_i2p.utils.path.abspath(
            wayround_i2p.utils.path.join(dirname, files[i])
            )
...
def files_recurcive_list(
    ...
    s_sep = wayround_i2p.utils.path.select_s_sep(dirname)
    ...
    dirname = wayround_i2p.utils.path.normpath(dirname)

    absp = dirname.startswith(s_sep)
    dirname = wayround_i2p.utils.path.abspath(dirname)
    ...
    if isinstance(exclude_paths, list):
        ...
        for i in exclude_paths:
            ...
            if i.startswith(s_sep):
                f_path = wayround_i2p.utils.path.abspath(i)
            else:
                f_path = wayround_i2p.utils.path.abspath(
    ...
    dirname_path_length = wayround_i2p.utils.path.path_length(dirname)
    ...
    for dire, dirs, files in os.walk(
        ...
        current_path_length = (
            wayround_i2p.utils.path.path_length(dire) - dirname_path_length
            ) + 1
        ...
        for f in files:
            ...
            f_path = wayround_i2p.utils.path.join(dire, f)
        ...
        if not mute:
            ...
            if not relative_to:
                ...
                pp = wayround_i2p.utils.path.relpath(dire, relative_to)
    ...
    if not relative_to:
        ...
        for i in range(len(ret)):
            ret[i] = wayround_i2p.utils.path.relpath(ret[i], relative_to)
...
def get_dir_size(name):
    name = wayround_i2p.utils.path.abspath(name)
    ...
    for i in lst:
        f_pth = wayround_i2p.utils.path.abspath(name + os.path.sep + i)
    ...
    if not os.path.exists(filename):
        ...
        if os.path.isdir(filename):
            ...
            if not os.path.islink(filename):
                ...
                lnk = None
                dir_name = wayround_i2p.utils.path.abspath(
                ...
                try:
                    ...
                    if lnk[0] != '/':
                        lnk = wayround_i2p.utils.path.abspath(
                            wayround_i2p.utils.path.join(dir_name, lnk)
                            )
...
def files_by_mask_copy_to_dir(in_dir, out_dir, mask='*.h'):
    ...
    in_dir = wayround_i2p.utils.path.abspath(in_dir)
    out_dir = wayround_i2p.utils.path.abspath(out_dir)
...
def which(name, under=None, exception_if_not_found=True):
    ...
    for i in under:
        ...
        n_f_n = wayround_i2p.utils.path.join(i, name)
...
def checksumed_dir_redue(reducing_dir, reducable_dir, method='sha512'):
    ...
    reducing_dir_filelist = wayround_i2p.utils.file.files_recurcive_list(
        reducing_dir,
        onerror=None,
        followlinks=False,
        exclude_paths=None,
    ...
    reducing_dir_sums = wayround_i2p.utils.checksum.checksums_by_list(
        reducing_dir_filelist, method
        )

    reducable_dir_filelist = wayround_i2p.utils.file.files_recurcive_list(
        reducable_dir,
        onerror=None,
        followlinks=False,
        exclude_paths=None,
    ...
    reducable_dir_sums = wayround_i2p.utils.checksum.checksums_by_list(
    ...
    for i in reducable_dir_filelist:
        ...
        for j in reducing_dir_filelist:
            ...
            if reducing_dir_sums[j] == reducable_dir_sums[i]:
                ...
                rel_path = \
                    wayround_i2p.utils.path.relpath(j, os.path.dirname(i))
