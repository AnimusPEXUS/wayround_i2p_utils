...
def source_version_comparator(
    ...
    d1 = wayround_i2p.utils.tarball.parse_tarball_name(
        name1,
        True,
        acceptable_source_name_extensions
        )

    d2 = wayround_i2p.utils.tarball.parse_tarball_name(
        name2,
        True,
        acceptable_source_name_extensions
        )
...
def same_base_structurize_by_version(bases):
    ...
    ver_tree = wayround_i2p.utils.directory.Directory()
...
def get_bases_from_ver_tree(directory, preferred_tarball_compressors):
    ...
    for path, dirs, files in directory.walk():
        for i in files:
            ...
            res = wayround_i2p.utils.tarball.\
                tarball_names_list_subdivide_by_status(
                    val
                )

            for key in res.keys():
                res2 = wayround_i2p.utils.path.select_by_prefered_extension(
                    res[key],
