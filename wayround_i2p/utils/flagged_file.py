...
class FlaggedFile:
    ...
    def __init__(
        ...
        if object_locker is None:
            object_locker = wayround_i2p.utils.threading.ObjectLocker()
    ...
    def get_str_list(self, name):
        ret = self.get_flag_data(name)
        if not wayround_i2p.utils.types_presets.is_list_of_str(ret):
            ret = []
        return ret

    def set_str_list(self, name, value):
        if not wayround_i2p.utils.types_presets.is_list_of_str(value):
    ...
    def get_str_set(self, name):
        ret = self.get_flag_data(name)
        if not wayround_i2p.utils.types_presets.is_set_of_str(ret):
            ret = set()
        return ret

    def set_str_set(self, name, value):
        if not wayround_i2p.utils.types_presets.is_set_of_str(value):
    ...
    def get_int_list(self, name):
        ret = self.get_flag_data(name)
        if not wayround_i2p.utils.types_presets.is_list_of_int(ret):
            ret = []
        return ret

    def set_int_list(self, name, value):
        if not wayround_i2p.utils.types_presets.is_list_of_int(value):
    ...
    def set_int_list_n(self, name, value):
        if (value is not None
            and not wayround_i2p.utils.types_presets.is_list_of_int(value)
            ):
            raise TypeError(
