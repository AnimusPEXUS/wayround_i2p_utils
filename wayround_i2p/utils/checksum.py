...
def make_dir_checksums_fo(
    ...
    rel_to = wayround_i2p.utils.path.abspath(rel_to)
    ...
    if not os.path.isdir(dirname):
        ...
        if not hasattr(output_fileobj, 'write'):
            ...
            for wres in os.walk(dirname):
                ...
                for f in files:
                    root_f = wayround_i2p.utils.path.join(root, f)

                    if exclude is not None and root_f in exclude:
                        continue

                    rel_path = wayround_i2p.utils.path.relpath(
                        root_f, rel_to
                        )
