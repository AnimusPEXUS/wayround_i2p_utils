...
import wayround_i2p.utils.directory
...
class HTMLWalk:
    ...
    def __init__(self, domain, scheme='https', port=None):
        ...
        self._port = port
        self._domain = domain
        self._tree = wayround_i2p.utils.directory.Directory()
    ...
    def _listdir(self, path, variant=0):
        ...
        if isinstance(path, str):
            path = wayround_i2p.utils.path.split(
                path,
                end_slash='eat'
                )
    ...
    def search_objects(self, path_lst):
        ...
        path_lst_j = wayround_i2p.utils.path.join(path_lst)
    ...
    def populate_tree_from_search_objects_result(self, path_lst):
        ...
        if search_objects_res is not None:
            for i in search_objects_res:
                ...
                if not wayround_i2p.utils.uri.isuri(i_unquoted):

                    is_dir = i_unquoted.endswith('/')

                    i_unquoted_norm = wayround_i2p.utils.path.normpath(
                        i_unquoted
                        )
                    ...
                    if i_unquoted_norm_dirname == '':
                        i_unquoted_norm = wayround_i2p.utils.path.join(
                            path_lst,
                    ...
                    i_unquoted_norm_dirname_splitted = \
                        wayround_i2p.utils.path.split(
                            i_unquoted_norm_dirname
                        )

                    if (wayround_i2p.utils.path.join(path_lst)
