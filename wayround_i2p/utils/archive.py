...
import wayround_i2p.utils.exec
...
def _extract_zip(file_name, output_dir):
    ...
    try:
        proc = wayround_i2p.utils.exec.simple_exec(
...
def _extract_tar_7z(file_name, output_dir):
    ...
    try:
        proc_7z = wayround_i2p.utils.exec.simple_exec(
            '7z',
        ...
        try:
            proc_tar = wayround_i2p.utils.exec.simple_exec(
                'tar',
...
def canonical_compressor(
    ...
    #'''
    ret = wayround_i2p.utils.exec.process_stream(
        compressor,
        stdin=stdin,
        stdout=stdout,
        stderr=stderr,
...
def archive_tar_canonical_fobj(
    ...
    if not os.path.isdir(dirname):
        ...
        try:
            tarproc = wayround_i2p.utils.exec.simple_exec(
                'tar',
                options=options,
...
def extract_tar_canonical_fobj(
    ...
    ret = wayround_i2p.utils.file.create_if_not_exists_dir(dirname)
        ...
        try:
            ...
            try:
                if not isinstance(fobj, tarfile.ExFileObject):
                    ...
                    wayround_i2p.utils.stream.cat(fobj, fd)
            finally:
                fobj.close()
        finally:
            fd.close()
...
def extract_low(
    ...
    if cleanup_output_dir:
        ...
        for i in outdir_files:
            ij = wayround_i2p.utils.path.join(outdir, i)
            wayround_i2p.utils.file.remove_if_exists(ij)
...
def tarobj_check_member_sum(tarobj, sums, member_name):
    ...
    if not isinstance(fobj, tarfile.ExFileObject):
        ...
        summ = wayround_i2p.utils.checksum.make_fileobj_checksum(fobj)
