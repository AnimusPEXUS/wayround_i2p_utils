...
import wayround_i2p.utils.weakref
...
class LogStrong:
    ...
    def __init__(
        ...
        self._group, self._user = wayround_i2p.utils.osutils.convert_gid_uid(
            group,
            user
            )
        ...
        if ret == 0:
            ...
            if timestamp is None:
                timestamp = wayround_i2p.utils.time.currenttime_stamp_iso8601()

            self.filename = wayround_i2p.utils.path.abspath(
                os.path.join(
                    log_dir,
    ...
    def stop(self, echo=True):
        ...
        with self._stop_lock:
            ...
            if not self._stop_called_once:
                ...
                timestamp = \
                    wayround_i2p.utils.time.currenttime_stamp_iso8601()
    ...
    def exception(self, text, echo=True, timestamp=None, tb=True):
        ei = wayround_i2p.utils.error.return_instant_exception_info(
            tb=tb
            )
        ttt = """\
{text}
