...
import wayround_i2p.utils.logging
...
def logging_setup(loglevel='INFO'):
    ...
    opts = wayround_i2p.utils.getopt.getopt_keyed(sys.argv[1:])[0]
    ...
    if '--loglevel' in opts:
        ...
        if not log_level_u in wayround_i2p.utils.logging.LEVEL_NAMES:
...
def command_processor(
    ...
    opts, args = wayround_i2p.utils.getopt.getopt_keyed(opts_and_args_list)
        ...
        if callable(subtree):
            ...
            if show_help:
                ...
                try:
                    ...
                    ex_txt = wayround_i2p.utils.error.return_exception_info(
                        e,
                        tb=True
                        )
...
class MainScript:
    ...
    def main(self):
        ...
        commands = wayround_i2p.tasktracker.commands.commands()

        ret = wayround_i2p.utils.program.program('wro-tasktracker', commands, None)
        return
