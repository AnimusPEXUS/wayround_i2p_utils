...
import wayround_i2p.utils.dict
...
def render_attributes(indict, path=[], tagname='', xml_indent_size=2):
    ...
    indent = wayround_i2p.utils.text.fill(' ', inaddr_l * xml_indent_size)
    nameindent = wayround_i2p.utils.text.fill(' ', len(tagname))
...
class DictTreeToXMLRenderer:
    ...
    def __init__(
        ...
        self.xml_indent = wayround_i2p.utils.text.fill(' ', xml_indent_size)
    ...
    def _place_found_css_or_js(
        ...
        if isinstance(self.css_and_js_holder['content'], dict):
            wayround_i2p.utils.dict.append(
    ...
    def _render(self, root, indict, path=[]):
        ...
        indent = wayround_i2p.utils.text.fill(
            ' ',
