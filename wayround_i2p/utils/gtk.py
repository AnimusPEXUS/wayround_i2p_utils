...
try:
    ...
    class MessageDialog(Gtk.MessageDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.wayround_i2p_response = Gtk.ResponseType.NONE
            self.wayround_i2p_iteration_loop = GtkIteratedLoop()

        def run(self):

            self.show_all()

            self.connect('close', self.wayround_i2p_close_listener)
            self.connect('response', self.wayround_i2p_response_listener)

            self.wayround_i2p_iteration_loop.wait()

            return self.wayround_i2p_response

        def destroy(self, *args, **kwargs):
            self.wayround_i2p_iteration_loop.stop()
            return super().destroy(*args, **kwargs)

        def wayround_i2p_response_listener(self, dialog, response_id):
            self.wayround_i2p_response = response_id
            self.wayround_i2p_iteration_loop.stop()

        def wayround_i2p_close_listener(self, dialog):
            self.wayround_i2p_iteration_loop.stop()
    ...
    class DirectoryTreeView(Gtk.TreeView):
        ...
        def load_dir(self, itera, path):
            ...
            for i in lst:
                if os.path.isdir(
                        wayround_i2p.utils.path.join(path, i)
                        ):
